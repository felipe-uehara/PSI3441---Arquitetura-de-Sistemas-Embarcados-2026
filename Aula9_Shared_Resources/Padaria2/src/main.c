...
K_MUTEX_DEFINE(vitrine); //variável que antecede o acesso à variável saldo_vitrine, caso ela seja bloqueada, não se acessa o saldo
...
void cliente_thread_entry(void *p1, void *p2, void *p3) {
    while (1) {
        ...
        if (saldo_vitrine > 0) {
            ...
            // Se o cliente chegar e não tiver pão, ele avisa
            printk("   -> [CLIENTE] Queria pao, mas a vitrine esta vazia!\n");
        }
